...
def monitor(options, log='', local=False):
   ...
   #     ROOT.gROOT.SetBatch()
    ...
    if options.job_maxtime is not None:
       if BATCH_HTC: ADD_OPTIONS += ['-append "+MaxRuntime = '+str(options.job_maxtime)+'"']
...
#              if options.verbose: print output_file
              ...
              if options.check_err:
                 ...
                 if '/' in input_file_woEXT:
                  #   err_files_wildcard = input_file_woEXT[:input_file_woEXT.rfind('/')]+'/'+options.batch+input_file_woEXT[input_file_woEXT.rfind('/'):]
                    err_files_wildcard = input_file_woEXT[:input_file_woEXT.rfind('/')]+'/logs'+input_file_woEXT[input_file_woEXT.rfind('/'):]
                 else:
                  #   err_files_wildcard = options.batch+'/'+input_file_woEXT
                    err_files_wildcard = 'logs/'+input_file_woEXT
               ...
               #   if (len(err_files) > 0) and (os.stat(err_files[-1]).st_size > 0):
                 if "No result file found for prefix" in open(err_files[-1]).read():
                  ...
                  #   RESUB_FILES += [input_file]
                    ...
                    print (colored_text('> we have to resubmit (non-empty err file)', ['93']), colored_text(os.path.relpath(input_file), ['1', '93']))
            ...
            #   if options.check_root:
              ...
              if options.verbose: print (colored_text('> job still running', ['93']), colored_text(os.path.relpath(input_file), ['1', '93']))
              counter_running += 1
              continue
...
if __name__ == '__main__':
    ...
    # parser.add_argument('-t', '--job-maxtime', dest='job_maxtime', action='store', default=32400,
    ...
    parser.add_argument('-j', '--jobs-max', dest='jobs_max', action='store', type=int, default=4950,
